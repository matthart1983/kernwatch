...
[Back to the README](../README.md). Detailed setup, controls, tracing, recording and development guidance.
...
kernwatch is under active development. The [gap audit](GAPS.md) and [requirement ledger](COMPLETION.md) distinguish implemented features, environment restrictions, and remaining work.
...
Demo mode needs no administrator privileges, starts no host probes and does not change host controls. Run `--demo` for manual exploration. See the [demo guide](DEMO.md) for the scene walkthrough and preview generation.
...
| `F` | [Flame](FLAME_GRAPHS.md) | CPU/syscall stack capture, zoomable icicles, and baseline comparison |
...
The tests cover parsing, task identity, navigation, replay, reports, action workflows, and Ratatui layout buffers. The GitHub Actions workflow is configured for these checks, including terminal and guided-demo smoke tests. Privileged kernel validation is separate; see [validation evidence and its scope](VALIDATION.md).
...
The [v0.4.0 performance review](PERFORMANCE_REVIEW.md) includes host CPU/RSS
...
See the [Syscalls/eBPF review](SYSCALL_EBPF_REVIEW.md) for the latest tab findings and the [gap audit](GAPS.md) for scope and the [specification](SPEC.md) for the intended product.
