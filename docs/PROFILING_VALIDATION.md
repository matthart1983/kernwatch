...
## Native measurement sample

The final CI benchmark recorded these results on four-CPU runners. CPU values
are percentages of one logical CPU. Throughput is sampled / unsampled with the
same worker count. RSS is process resident memory in KiB before / after capture;
it includes workload threads and mapped pages and is not total kernel BPF memory.
Captures run sequentially, so allocator reuse affects subsequent baselines.

| Runner | Hz | Ingest CPU | BPF CPU | Throughput ratio | RSS before / after (KiB) |
| --- | ---: | ---: | ---: | ---: | ---: |
| ARM64 | 49 | 0.51% | 0.008% | 1.001 | 2,312 / 49,372 |
| ARM64 | 99 | 0.56% | 0.025% | 1.002 | 16,596 / 62,180 |
| x86-64 | 49 | 0.66% | 0.042% | 0.999 | 2,876 / 60,076 |
| x86-64 | 99 | 0.81% | 0.101% | 0.982 | 27,300 / 74,108 |

These are single-run measurements of the controlled workload, not population
estimates or production overhead promises. The native logs are available in the
linked CI run; the benchmark prints these metrics on every execution.
...
`7610292f63bbbdc8a5257bcdf5bb9cec6f6d04a6`. The complete
[CI run](https://github.com/matthart1983/kernwatch/actions/runs/34677615128)
