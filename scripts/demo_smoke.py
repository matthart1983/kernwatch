...
# Read the scene count from the tour, so adding a scene cannot leave this
# assertion matching a stale binary instead of the one under test.
...
with tempfile.TemporaryDirectory(prefix='kernwatch-demo-test-') as work:
    ...
    try:
        drain(0.8)
        assert f'DEMO TOUR 1/{scenes}'.encode() in output,f'guided demo did not start with {scenes} scenes'
        drain(5.2)
