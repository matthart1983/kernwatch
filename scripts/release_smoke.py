...
with tempfile.TemporaryDirectory(prefix='kernwatch-release-') as work:
    ...
    def run(*args):
        return subprocess.run([str(binary), *args], cwd=work, env=env, check=True, capture_output=True, text=True).stdout
    ...
    if 'linux' not in target:
        result = subprocess.run([str(binary), '--snapshot'], cwd=work, env=env, capture_output=True, text=True)
